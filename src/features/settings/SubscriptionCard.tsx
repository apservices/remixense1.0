...
import { Crown, Zap, ExternalLink, Loader2, Check } from "lucide-react";
...
export default function SubscriptionCard() {
  ...
  const currentPlan: PlanType = (subscription?.status === 'active' ? 'premium' : 'free');
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div className="p-3 rounded-lg bg-muted/30">
            ...
            <p className="text-lg font-semibold">{features.maxTracks === -1 ? 'Ilimitado' : features.maxTracks}</p>
          </div>
          <div className="p-3 rounded-lg bg-muted/30">
            <p className="text-sm text-muted-foreground">Storage</p>
            <p className="text-lg font-semibold">{features.maxStorage >= 1000 ? `${features.maxStorage / 1000} GB` : `${features.maxStorage} MB`}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span>Tracks</span>
              <span className="text-muted-foreground">{usage.tracks} / {features.maxTracks === -1 ? '∞' : features.maxTracks}</span>
            </div>
            <Progress value={getUsagePercent(usage.tracks, features.maxTracks)} />
          </div>
        </div>

        <ul className="space-y-1">
          {[
            { label: 'Stems AI', enabled: features.stemsEnabled },
            { label: 'Export HD', enabled: features.exportHD },
            { label: 'Marketplace', enabled: features.marketplace }
          ].map(({ label, enabled }) => (
